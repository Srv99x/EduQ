...
interface StudySession {
    ...
    dayKey: string; // Local date key YYYY-MM-DD based on session start
}
...
export const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
      activeStudyRef.current = activeStudyStart;
  }, [activeStudyStart]);
  ...
  useEffect(() => {
      ...
      const update = () => {
          setActiveElapsedSeconds(Math.max(0, Math.floor((Date.now() - activeStudyStart) / 1000)));
      };
  ...
  useEffect(() => {
      const autoStopOnLeave = () => {
          ...
          try {
              const existingRaw = localStorage.getItem(STUDY_SESSIONS_KEY);
              const existing: StudySession[] = existingRaw ? JSON.parse(existingRaw) : [];
              localStorage.setItem(STUDY_SESSIONS_KEY, JSON.stringify([...existing, newSession]));
          } catch {
              // Intentionally no-op; app should still clear active session marker.
          }

          localStorage.removeItem(STUDY_ACTIVE_KEY);
      };

      window.addEventListener('beforeunload', autoStopOnLeave);
      window.addEventListener('pagehide', autoStopOnLeave);

      return () => {
          window.removeEventListener('beforeunload', autoStopOnLeave);
          window.removeEventListener('pagehide', autoStopOnLeave);
      };
  }, []);
  ...
  const stopStudySession = (endTime = Date.now()) => {
      ...
      const newSession: StudySession = {
          id: `s_${activeStudyStart}`,
          startedAt: activeStudyStart,
          endedAt: endTime,
          durationSeconds: duration,
          dayKey: getLocalDayKey(activeStudyStart),
      };
  ...
  const startStudySession = () => {
      ...
      // Find most recently modified notebook and continue there.
        ...
        <div>
          <h1 className="text-3xl font-bold text-white mb-2">Hello, {user.name} 👋</h1>
                    <p className="text-subtext">
                        ...
                        {activeStudyStart ? ` • Studying now: ${formatDuration(activeElapsedSeconds)}` : ''}
                        {` • Sessions today: ${sessionsTodayCount}`}
                    </p>
        </div>
        <div className="flex gap-3">
            <Button variant="secondary" size="md" onClick={() => setShowSchedule(true)}>VIEW SCHEDULE</Button>
                        <Button variant="primary" size="md" onClick={handleResumeStudy}>
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={data}>
                            ...
                            <Tooltip 
                                ...
                                labelStyle={{ color: '#D8DCE5', fontWeight: 600 }}
                            ...
                            <Bar dataKey="hours" radius={[6, 6, 6, 6]}>
                                {data.map((entry, index) => (
                                    <Cell
                                        key={`cell-${index}`}
                                        fill={['#4ADE80', '#F87171', '#FACC15', '#FB923C', '#22D3EE', '#A78BFA', '#60A5FA'][index % 7]}
                                    />
                                ))}
                            </Bar>
                        </BarChart>
